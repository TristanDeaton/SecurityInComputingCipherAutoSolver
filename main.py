...
def main():
    ...
    while(notDone):
        ...
        if(int(decision) == 1):
            ...
            bunchOStrings = [cipherTextCaesar, cipherTextMonoAlphabetic, cipherTextViginere]

            randomStringFromList = bunchOStrings[random.randint(0,len(bunchOStrings))]
            viginereOutput = viginereBreaker.viginereBreaker(randomStringFromList)
            ...
            viginereOutput = viginereBreaker.viginereBreaker(testingString)
            viginerePercentage = getPercentageSpelledRight(viginereOutput)
